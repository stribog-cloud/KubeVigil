...
func TestProvenanceChecker_Metadata(t *testing.T) {
	c := &ProvenanceChecker{}

	assert.Equal(t, "image-provenance", c.Name())
...
func TestProvenanceChecker_Run(t *testing.T) {
	c := &ProvenanceChecker{}
	...
	tests := []struct {
		...
		{
			name: "RequireProvenance false returns no findings (NO-OP)",
			setup: func() *checker.ResourceCache {
				cache := checker.NewResourceCache()
				cache.SetPolicies(&checker.Policies{
					Images: checker.ImagePolicies{
		...
		{
			name: "fixture: pod-no-digest.yaml triggers when provenance required",
			setup: func() *checker.ResourceCache {
				cache := helpers.LoadFixture(t, "image-provenance", "pod-no-digest.yaml")
				cache.SetPolicies(&checker.Policies{
					Images: checker.ImagePolicies{
						RequireProvenance: true,
					},
		...
		},
		{
			...
			setup: func() *checker.ResourceCache {
				cache := helpers.LoadFixture(t, "image-provenance", "pod-with-digest.yaml")
				cache.SetPolicies(&checker.Policies{
					Images: checker.ImagePolicies{
						RequireProvenance: true,
					},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantFindings > 0 {
				...
				assert.Equal(t, "image-provenance", findings[0].Checker)
				...
				assert.Contains(t, findings[0].Message, "provenance verification")
...
func TestProvenanceChecker_CancelledContext(t *testing.T) {
	c := &ProvenanceChecker{}
...
func TestProvenanceChecker_FindingFields(t *testing.T) {
	c := &ProvenanceChecker{}
	ctx := context.Background()
	...
	f := findings[0]
	assert.Equal(t, "image-provenance", f.Checker)
	...
	assert.Contains(t, f.Remediation, "SLSA")
	assert.Equal(t, ".spec.containers[0].image", f.FieldPath)
}

func TestProvenanceChecker_AllWorkloadTypes(t *testing.T) {
	c := &ProvenanceChecker{}
	ctx := context.Background()
