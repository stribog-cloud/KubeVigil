...
func TestSignatureVerificationChecker_Metadata(t *testing.T) {
	c := &SignatureVerificationChecker{}

	assert.Equal(t, "image-signature-verification", c.Name())
...
func TestSignatureVerificationChecker_Run(t *testing.T) {
	c := &SignatureVerificationChecker{}
	...
	tests := []struct {
		...
		{
			name: "RequireSignatures false returns no findings (NO-OP)",
			setup: func() *checker.ResourceCache {
				cache := checker.NewResourceCache()
				cache.SetPolicies(&checker.Policies{
					Images: checker.ImagePolicies{
		...
		{
			name: "fixture: pod-no-digest.yaml triggers when signatures required",
			setup: func() *checker.ResourceCache {
				cache := helpers.LoadFixture(t, "image-signature-verification", "pod-no-digest.yaml")
				cache.SetPolicies(&checker.Policies{
					Images: checker.ImagePolicies{
						RequireSignatures: true,
					},
		...
		},
		{
			...
			setup: func() *checker.ResourceCache {
				cache := helpers.LoadFixture(t, "image-signature-verification", "pod-with-digest.yaml")
				cache.SetPolicies(&checker.Policies{
					Images: checker.ImagePolicies{
						RequireSignatures: true,
					},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantFindings > 0 {
				...
				assert.Equal(t, "image-signature-verification", findings[0].Checker)
				...
				assert.Contains(t, findings[0].Message, "signature verification")
...
func TestSignatureVerificationChecker_CancelledContext(t *testing.T) {
	c := &SignatureVerificationChecker{}
...
func TestSignatureVerificationChecker_FindingFields(t *testing.T) {
	c := &SignatureVerificationChecker{}
	ctx := context.Background()
	...
	f := findings[0]
	assert.Equal(t, "image-signature-verification", f.Checker)
	...
	assert.Contains(t, f.Remediation, "cosign")
	assert.Equal(t, ".spec.containers[0].image", f.FieldPath)
}

func TestSignatureVerificationChecker_AllWorkloadTypes(t *testing.T) {
	c := &SignatureVerificationChecker{}
	ctx := context.Background()
