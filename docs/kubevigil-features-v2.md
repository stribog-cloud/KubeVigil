# KubeVigil — Comprehensive Feature Specification v2
...
**Language:** Go
**Minimum Go Version:** 1.22+ (for enhanced standard library features)
...
### 7.1 Fix Capabilities

- **Manifest patching** — `kubevigil fix` rewrites YAML manifests to resolve findings. Preserves comments, formatting, key ordering, and blank lines using a YAML round-trip library (not marshal/unmarshal which destroys formatting).
- **Dry-run mode** — `--dry-run` shows unified diff of what would change. Colored diff output (green = added, red = removed).
- **Selective fixing** — Fix only specific checks (`--checks privileged,run-as-root`) or severity levels (`--severity critical,high`). Or fix a single finding by fingerprint.
- **Backup creation** — Always creates `.bak` files before modifying originals. Configurable backup directory.
- **kubectl patch generation** — `--output kubectl` generates copy-pasteable `kubectl patch` commands for live cluster remediation.
- **Kustomize overlay generation** — `--kustomize` generates a `security-overlay/` directory with a `kustomization.yaml` and strategic merge patches that can be applied as an overlay to your existing Kustomize structure.
- **Helm values generation** — For known chart patterns (common Helm value paths for securityContext, resources, etc.), generate a `security-values.yaml` that overrides insecure defaults.
- **GitOps PR generation** — `kubevigil fix --git-pr` creates a branch, commits fixes, and opens a PR/MR (GitHub, GitLab). Requires `gh` or `glab` CLI.

### 7.2 Fix Safety

- **Conservative defaults** — Only fix issues where the safe resolution is unambiguous. Fixes that could break functionality require `--force` or interactive confirmation.
- **Fix classification** — Each fix is classified as:
  - **Safe** — Zero risk of breaking (e.g., `privileged: false`, `readOnlyRootFilesystem: true`). Applied automatically.
  - **Likely safe** — Very low risk (e.g., adding `drop: ALL` capabilities, setting `runAsNonRoot: true`). Applied with warning.
  - **Potentially breaking** — Could impact functionality (e.g., setting specific `runAsUser`, dropping `NET_RAW`). Requires confirmation.
  - **Manual only** — Cannot auto-fix safely (e.g., RBAC restructuring, NetworkPolicy creation). Only generates guidance.
- **Confirmation prompts** — Interactive confirmation for each potentially-breaking fix.
- **Fix impact analysis** — Warn when a fix might break functionality. E.g., "Dropping NET_RAW capability will break containers using `ping`. Container `debug-tools` in Deployment `ops-toolkit` uses `ping` — consider keeping NET_RAW for this container."
- **Rollback instructions** — Every fix output includes rollback commands. For manifest fixes: `cp file.yaml.bak file.yaml`. For kubectl patches: reverse patch command.
- **Fix validation** — After applying fixes, optionally re-scan to verify the fix resolved the finding (--verify flag).
...
`kubevigil fix` with manifest patching, kubectl patch generation, dry-run mode, Kustomize overlay generation. Fix integration tests (scan → fix → re-scan → zero findings).

### Phase 4 — CI/CD (Weeks 12-14)
...
### Phase 5 — Continuous (Weeks 15-18)
...
### Phase 6 — Enterprise (Weeks 19-24)
...
### Phase 7 — Ecosystem (Weeks 25+)
Go SDK with public API, plugin system, check template generator, comprehensive docs site, Helm/Krew/Homebrew distribution, Cosign binary signing. Benchmark regression tracking, mutation testing gate.
...
| Phase 3 | File I/O, YAML round-tripping, diff algorithms, template generation (`text/template`), fuzzing (`testing.F`) |
| Phase 4 | Build tooling (GoReleaser, Makefiles, ldflags), CI scripting, SARIF format, golden file testing, property-based testing |
| Phase 5 | HTTP servers (`net/http`), webhook TLS, Prometheus client library, controller-runtime, admission review types, benchmark testing |
| Phase 6 | Database access (SQLite via modernc), concurrency patterns (channels, select, fan-out/fan-in), context propagation, OPA/Rego integration, mutation testing |
| Phase 7 | Plugin loading (`plugin` package), public API design, backward compatibility, documentation generation (godoc), release engineering |
...
│   │   ├── fixer.go + _test.go
│   │   ├── yaml_patcher.go + _test.go
│   │   ├── kubectl_gen.go + _test.go
│   │   └── kustomize_gen.go + _test.go
...
*This document is the complete feature and testing specification for KubeVigil v2. It defines the full vision from MVP to ecosystem. Implementation follows the phased roadmap — ship early, test thoroughly, iterate based on community feedback.*
