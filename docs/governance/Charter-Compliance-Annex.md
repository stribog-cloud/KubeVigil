...
| Per-package floor for critical paths | 96% for `internal/fix/`, `internal/mcp/`, `internal/checker/secrets/`, `internal/policy/`, `internal/baseline/`, `internal/webhook/`, **enforced** in CI via `make coverage` → `scripts/coverage-pkg.sh` |
...
| 1.3.0 | 7 | 2026-07-06 | v1.2.0 admission webhook: internal/webhook added to per-package floor; webhook command in public surface. |